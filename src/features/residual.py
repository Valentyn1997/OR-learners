...
from normflows.nets import LipschitzMLP, InducedNormLinear
...
InducedNormLinear.forward = new_forward
...
class ResidualTransform(TransformModule):
    ...
    def __init__(self, input_dim: int, hidden_dim: int, exact_trace=False, lipschitz_const=0.9, cache_size=0, reduce_memory=True,
                 atol=1e-5, rtol=1e-5):
        ...
        self.lnet = LipschitzMLP([self.input_dim, hidden_dim, self.input_dim],
                                 init_zeros=exact_trace, lipschitz_const=lipschitz_const, max_lipschitz_iter=None,
    ...
    def _inverse(self, y):
        return self.res_block._inverse_fixed_point(y, atol=self.atol, rtol=self.rtol)
